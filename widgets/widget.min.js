...
$(document).delegate('div[data-widget="plot.stacked"]',{update:function(d,a){var b=$(this).attr("data-label").explode(),c=$(this).attr("data-color").explode(),e=$(this).attr("data-exposure").explode(),f=$(this).attr("data-axis").explode();$(this).attr("data-zoom");for(var g=JSON.parse($(this).attr("data-stacklabels")),h=JSON.parse($(this).attr("data-correctdate")),m=[],k=0;k<a.length;k++){for(var n=a[k],p=0;p<n.length;p++){var q=n[p][0];h&&(q-=864E5);q=q.toString().substring(0,9).concat("0000");n[p][0]=
parseInt(q)}n.pop();a[k]=n;m[k]={type:"stair"!=e[k]?e[k]:"line",step:"stair"==e[k]?"left":!1,name:b[k],data:a[k],color:c[k]?c[k]:null}}$("#"+this.id).highcharts({chart:{type:"column"},series:m,xAxis:{type:"datetime",title:{text:f[0]}},yAxis:{min:$(this).attr("data-ymin"),max:$(this).attr("data-ymax"),title:{text:f[1]},stackLabels:{enabled:g,style:{color:"#fff","font-size":"13px","line-height":"14px"}}},plotOptions:{column:{stacking:"normal",dataLabels:{enabled:!1}}}})},point:function(d,a){var b=$(this).attr("data-count");
1>b&&(b=100);for(var c=0;c<a.length;c++)if(a[c]){for(var e=$("#"+this.id).highcharts(),f=0;f<a[c].length;f++)e.series[c].addPoint(a[c][f],!1,e.series[c].data.length>=b);e.redraw()}}});
...
e[f][1]],!1,g.series[f].data.length>=b);g.series[1].addPoint(a[2][f],!1,g.series[f].data.length>=b);g.redraw()}}});$(document).delegate('svg[data-widget="icon.light"]',{update:function(d,a){var b=Math.round(a[0]/$(this).attr("data-max")*10),c=1;$("#"+this.id+" g#light-rays line").each(function(){b>=c?$(this).css("visibility","visible"):$(this).css("visibility","hidden");c++})}});
$(document).delegate('svg[data-widget="icon.volume"]',{update:function(d,a){var b=Math.round(a[0]/$(this).attr("data-max")*71);fx.bar(this,b,[18,68],[89,50])}}); 
